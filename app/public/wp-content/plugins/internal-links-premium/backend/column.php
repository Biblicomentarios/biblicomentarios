...
class Column
{
        ...
        if (\ILJ\ilj_fs()->is__premium_only()) {
            if (\ILJ\ilj_fs()->can_use_premium_code() ) {
                ...
                foreach($taxonomies as $taxonomy) {
                       add_filter('manage_' . $taxonomy . '_custom_column', ['\ILJ\Backend\Column', 'addConfiguredLinksColumnContent__premium_only'], 15, 3);
                       add_filter('manage_edit-' . $taxonomy . '_columns', ['\ILJ\Backend\Column', 'addConfiguredLinksColumnHeader']);
                }

                $types[] = \ILJ\Posttypes\CustomLinks::ILJ_POSTTYPE_CUSTOM_LINKS_SLUG;
            }
        }

        foreach ($types as $type) {
            add_action('manage_' . $type . '_posts_custom_column', ['\ILJ\Backend\Column', 'addConfiguredLinksColumnContent'], 10, 2);
    ...
    public static function sortConfiguredLinksColumn()
    {
        ...
        if(self::ILJ_COLUMN_CONFIGURED_LINKS != $orderby) return;
    ...
    /**
     * Outputs the content of the configured links column on taxonomies
     *
     * @since 1.1.3
     * @param string $row         Blank string
     * @param string $column_name Name of the column
     * @param int    $term_id     Term ID
     *
     * @return string
     */
    public static function addConfiguredLinksColumnContent__premium_only($row, $column_name, $term_id)
    {
        $data = get_term_meta($term_id,  Postmeta::ILJ_META_KEY_LINKDEFINITION);
        return count($data) ? (string) count($data[0]) : '0';
    }
}