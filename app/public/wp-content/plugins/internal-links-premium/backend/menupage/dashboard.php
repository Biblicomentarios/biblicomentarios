...
class Dashboard extends AbstractMenuPage
{
        ...
        add_action(
            'admin_enqueue_scripts', function ($hook) {
                ...
                if ($screen->base === $this->page_hook) {
                    wp_register_script(self::ILJ_STATISTIC_HANDLE, ILJ_URL . 'admin/js/ilj_statistic.js', [], ILJ_VERSION);
    ...
    private function getIndexModeHint($building_duration)
    {
        ...
        $threshold_automatic_mode = 3.5; //Threshold in seconds
        $threshold_manual_mode = 15.5; //Threshold in seconds
        ...
        $message_documentation_link = sprintf('<a href="%s" target="_blank" rel="noopener">%s</a>', Help::getLinkUrl('index-generation-mode/', null, 'index size warning', 'dashboard'), __('Read more in our documentation', 'internal-links'));
        ...
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            if ($current_index_mode == IndexMode::MANUALLY) {
                if ($building_duration > $threshold_manual_mode) {
                    $message = $message_cli_mode;
                    $class = 'danger';
                }
            } elseif ($current_index_mode == IndexMode::NONE) {
                return '';
            }
        }
    ...
    {
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            return ILJ_VERSION . ' <span class="badge pro">Pro</span>';
        }
