...
class Tools extends AbstractMenuPage
{
    ...
    public function register()
    {
        ...
        wp_register_script('ilj_tools', ILJ_URL . 'admin/js/ilj_tools.js', [], ILJ_VERSION);
        wp_localize_script(
            'ilj_tools', 'ilj_tools', [
    ...
    protected static function getTranslation()
    {
        ...
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            $translation += [
            'error_import_selection' => __('Please select at least one type and one source to start the import.', 'internal-links'),
            ];
        }
    ...
    protected function getKeywordImport()
    {
        ...
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            $post_types_input = $this->getWhitelistedPostTypeSelect__premium_only();
        }
        ...
        if(\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            $button = sprintf('<button class="button button-primary ilj-import">%s</button>', __('Import', 'internal-links'));
        }
        $output .= $button;
        $output .= '<span class="spinner"></span>';
        $output .= '<div class="clear"></div>';
        ...
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            $term_types_input = $this->getWhitelistedTermTypeSelect__premium_only();
        }
    ...
    public function exportScreen($current_screen)
    {
        ...
        case 'keywords':
            ...
            if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
                Export::printCsvTerms__premium_only($option_empty);
                Export::printCsvCustomLinks__premium_only($option_empty);
            }
            die();
                break;
        }
    ...
    /**
     * Returns a (multi-) select field of whitelisted post types
     *
     * @since  1.2.0
     * @return string
     */
    protected function getWhitelistedPostTypeSelect__premium_only()
    {
        $output = '';
        ...
        if (count($whitelisted_post_types)) {
            $output .= '<select name="ilj-import-intern-type" id="ilj-import-intern-type-post" multiple="multiple">';
            ...
            $output .= __('No whitelisted post types available.', 'internal-links');
        }
    ...
    /**
     * Returns a (multi-) select field of whitelisted term types
     *
     * @since  1.2.0
     * @return string
     */
    protected function getWhitelistedTermTypeSelect__premium_only()
    {
        $output = '';

        $whitelisted_term_types = \ILJ\Core\Options::getOption(TaxonomyWhitelist::getKey());
        $editor_taxonomy_types = TaxonomyWhitelist::getTaxonomyTypes();

        if (count($whitelisted_term_types)) {
            $output .= '<select name="ilj-import-intern-type" id="ilj-import-intern-type-term" multiple="multiple">';
            foreach ($editor_taxonomy_types as $taxonomy_type) {
                if (!in_array($taxonomy_type->name, $whitelisted_term_types)) {
                    continue;
                }

                $output .= '<option value="' . $taxonomy_type->name . '">' . $taxonomy_type->label . '</option>';
            }
            $output .= '</select> ';
        } else {
            $output .= __('No whitelisted taxonomy types available.', 'internal-links');
        }
    ...
    /**
     * Gets a keyword list from posts by their title (for importing)
     *
     * @since 1.2.0
     * @param \WP_Post $post The post from which keywords are extracted
     *
     * @return KeywordList
     */
    public static function getImportKeywordsByPostTitle__premium_only(\WP_Post $post)
    {
