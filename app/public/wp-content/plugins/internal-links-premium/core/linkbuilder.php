...
class LinkBuilder
{
    ...
    private function setupInLinks()
    {
        ...
        foreach ($post_rules as $post_rule) $this->link_ruleset->addRule(Regex::escapeDot($post_rule->anchor), $post_rule->link_to, $post_rule->type_to);
    ...
    private function maskLinkRules()
    {
        ...
        while ($this->link_ruleset->hasRule()) {
            ...
            if (($links_per_target > 0 && array_key_exists($link_rule->value, $link_target_count) && $link_target_count[$link_rule->value] >= $links_per_target)
                || (!$multi_keyword_mode && in_array($link_rule->pattern, $used_pattern))
            ) {
            ...
            foreach($phrases as $index => $rule)  {
                if (($links_per_target > 0
                    && array_key_exists($link_rule->value, $link_target_count)
                    && $link_target_count[$link_rule->value] == $links_per_target)
                    || (!$multi_keyword_mode && in_array($link_rule->pattern, $used_pattern))
                ) {
                    $this->link_ruleset->nextRule();
                    continue 2;
                ...
                $this->content = preg_replace('/' . Encoding::maskPattern($rule). '/u', $rule_id, $this->content, ($multi_keyword_mode ? -1 : 1));
    ...
    private function generateLink($link_rule, $anchor)
    {
                ...
                if ($link_rule->type != 'post') {
                    ...
                    case "custom":
                        ...
                        $nofollow = get_post_meta(
                            $link_rule->value,
                            \ILJ\Posttypes\CustomLinks::ILJ_FIELD_CUSTOM_LINKS_OPTION_NOFOLLOW,
                            true
                        );

                        $new_window = get_post_meta(
                            $link_rule->value,
                            \ILJ\Posttypes\CustomLinks::ILJ_FIELD_CUSTOM_LINKS_OPTION_NEW_WINDOW,
                            true
                        );

                        $template = Options::getOption(\ILJ\Core\Options\LinkOutputCustom::getKey());

                        if ($template == "") {
                            $template = \ILJ\Core\Options\LinkOutputCustom::getDefault();
                        } else {
                            $template = wp_specialchars_decode($template, \ENT_QUOTES);
                        }

                        break;
                    case "term":
                        $term = get_term($link_rule->value);
        ...
        if (\ILJ\ilj_fs()->is__premium_only()) {
            if (\ILJ\ilj_fs()->can_use_premium_code()) {
                if ($nofollow || $new_window) {
                    $replacment = '<a rel="' . ($nofollow ? 'nofollow' . ($new_window ? ' ' : '') : '') . ($new_window ? 'noopener' : '') . '" ' . ($new_window ? 'target="_blank" ' : '');
                    $link       = str_replace('<a ', $replacment, $link);
                }

                $check_nofollow = false;
            }
        }
