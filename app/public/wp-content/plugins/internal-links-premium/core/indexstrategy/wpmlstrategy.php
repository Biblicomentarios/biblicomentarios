...
class WPMLStrategy extends DefaultStrategy
{
    ...
    public function setIndices()
    {
        ...
        if (\ILJ\ilj_fs()->is__premium_only()) {
            if (\ILJ\ilj_fs()->can_use_premium_code()) {
                $terms = $this->loadTaxonomies__premium_only();
    ...
    /**
     * Get all relational translation meta data for taxonomies
     *
     * @since 1.2.0
     *
     * @return array
     */
    protected function getTranslationTaxonomies__premium_only()
    {
        if (!isset($this->translation_taxonomies)) {
            $this->translation_taxonomies = Translations::getByElementType('tax');
        }
        return $this->translation_taxonomies;
    }
                ...
                while ($this->link_rules[$language]->hasRule()) {
                    ...
                    if ($link_rule->value != $item->{$fields['id']}) {
                        ...
                        if (isset($rule_match['phrase'])) {
                            ...
                            if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
                                $limit_incoming_links = IndexAsset::getLimitLinksMetas__premium_only($link_rule->value , $data_type);
                                
                                if($limit_incoming_links[Editor::ILJ_META_KEY_LIMITINCOMINGLINKS] == true && $incoming_link[$link_rule->value] >= $limit_incoming_links[Editor::ILJ_META_KEY_MAXINCOMINGLINKS] && $limit_incoming_links[Editor::ILJ_META_KEY_MAXINCOMINGLINKS] != 0 ){
                                    $this->link_rules[$language]->nextRule();
                                    continue;
                                }
    ...
    protected function getDataLanguage($data_id, $data_type)
    {
        if (\ILJ\ilj_fs()->is__premium_only()) {
            if (\ILJ\ilj_fs()->can_use_premium_code() ) {
                if ($data_type == 'term' ) {
                    $translations_data = $this->getTranslationTaxonomies__premium_only();
                }
            }
        }
     ...
     * @inheritdoc
     */
    protected function loadTaxonomies__premium_only()
    {
        global $sitepress;
        $terms = [];
        ...
        foreach($this->languages as $language) {
            ...
            'exclude'  => Options::getOption(\ILJ\Core\Options\TermBlacklist::getKey()),
            'hide_empty' => false
            ];
            $taxonomies = new \WP_Term_Query($args);

            if (!$taxonomies || !$taxonomies->terms) {
                continue;
            }

            $terms = array_merge($terms, $taxonomies->terms);
        }
