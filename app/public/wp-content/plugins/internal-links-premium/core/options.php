    ...
    public function __construct()
    {
        $this->sections = [
            ...
            self::ILJ_OPTION_SECTION_CONTENT => [
                'options'     => [new Options\Whitelist(), new Options\TaxonomyWhitelist(), new Options\Blacklist(), new Options\BlacklistChildPages(), new Options\TermBlacklist(), new Options\KeywordOrder(), new Options\LinksPerPage(), new Options\LinksPerTarget(), new Options\MultipleKeywords(), new Options\NoLinkTags(), new Options\RespectExistingLinks() , new Options\LimitTaxonomyList()]
            ],
    ...
    public static function importOptions(array $options)
    {
        ...
        foreach($options as $key => $value) {
	        ...
	        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
	        	$to_sanitize[] = LinkOutputCustom::getKey();
	        }
    ...
    public static function exportOptions()
    {
        ...
        foreach ($options->sections as $section) {
            foreach ($section['options'] as $option) {
                ...
                if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
                    $escaped_option_values[] = LinkOutputCustom::getKey();
                }
