...
class Linkindex
{
    ...
    public static function getGroupedCount($column)
    {
        ...
        $type_mapping = ['link_from' => 'type_from', 'link_to' => 'type_to'];
    ...
    /**
     * Find all links by their anchor text
     *
     * @since  1.1.0
     * @param  string $anchor The anchor text to look up after
     * @return array
     */
    public static function getByAnchor__premium_only($anchor)
    {
        global $wpdb;

        $query = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . self::ILJ_DATABASE_TABLE_LINKINDEX . " linkindex WHERE linkindex.anchor = %s", $anchor);
