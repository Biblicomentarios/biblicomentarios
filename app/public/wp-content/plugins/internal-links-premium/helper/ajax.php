...
class Ajax
{
     ...
     * @return void
     */
    public static function indexRebuildAction__premium_only()
    {
        ...
        try {
            ...
            $duration = number_format($index_stat['last_update']['duration'], 2);
            $entries  = $index_stat['last_update']['entries'];

            $response = [
                'status'  => 'success',
                'message' => sprintf('<div class="notice notice-success is-dismissible"><p><strong>Internal Link Juicer:</strong></p><p class="message">' . __('Index was successfully rebuilt. The process took %s seconds for %d entries.', 'internal-links') . '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' . $hide_notification . '</span></button></div>', $duration, $entries)
            ];
        } catch (\Exception $e) {
            $response = [
                'status'  => 'error',
                'message' => sprintf('<div class="notice notice-error is-dismissible"><p><strong>Internal Link Juicer:</strong></p><p class="message">' . __('Unknown error - please contact support.', 'internal-links') . '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' . $hide_notification . '</span></button></div>', $duration, $entries)
            ];
        }
    ...
    public static function renderAnchorDetailStatisticAction()
    {
        ...
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            $anchor = wp_unslash((string) $_POST['anchor']);
            ...
            $data = '<table class="ilj-statistic-detail-table display"><thead><tr><th>' . __('Source', 'internal-links') . '</th><th>' . __('Source type', 'internal-links') . '</th><th>' . __('Target', 'internal-links') . '</th><th>' . __('Target type', 'internal-links') . '</th></tr></thead><tbody>';
            $row_counter = 0;
            ...
            foreach ($anchor_links as $link) {
                ...
                $data .= '<tr class="' . (( $row_counter % 2 === 0 ) ? 'even' : 'odd') . '"><td><a href="' . $asset_data_from->url . '" target="_blank" rel="noopener">' . $asset_data_from->title . '</a></td><td class="type"><span data-type="' .  $link->type_from . '">' . $type_from_detailed . '</span></td><td><a href="' . $asset_data_to->url . '" target="_blank" rel="noopener">' . $asset_data_to->title . '</td><td class="type"><span data-type="' .  $link->type_to . '">' . $type_to_detailed . '</span></td></tr>';
    ...
    public static function importKeywordIntern__premium_only()
    {
        ...
        $type_selection = $_POST['type_selection'];
        $source_import = $_POST['source_import'];

        if (!count($type_selection) || !count($source_import)) {
            wp_send_json_error(null, 400);
        }
