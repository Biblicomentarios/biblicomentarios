...
class Statistic
{
    ...
    public static function getConfiguredKeywordsCount()
    {
        ...
        if (\ILJ\ilj_fs()->is__premium_only()) {
            if (\ILJ\ilj_fs()->can_use_premium_code()) {
                $termmeta = Termmeta::getAllLinkDefinitions();

                foreach ($termmeta as $meta) {
                    $keywords = get_term_meta($meta->term_id, Postmeta::ILJ_META_KEY_LINKDEFINITION, true);
                    $configuredKeywords = array_merge($configuredKeywords, $keywords);
                }
            }
        }
    ...
    public static function getConfiguredKeywordsCountForAsset($asset_id, $asset_type)
    {
        ...
        if (\ILJ\ilj_fs()->can_use_premium_code__premium_only()) {
            if ($asset_type == 'term') {
                $data = get_term_meta($asset_id, Postmeta::ILJ_META_KEY_LINKDEFINITION);
     ...
     * @return     array
     */
    public static function getAggregatedCount($args = [])
    {
        $defaults = [
        "type"  => "link_from",
        "limit" => 10
