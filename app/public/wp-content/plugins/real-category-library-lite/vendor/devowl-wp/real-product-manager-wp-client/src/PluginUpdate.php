...
use DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\announcement\AnnouncementPool;
use DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\PluginUpdateChecker;
use DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\PluginUpdateLicensePool;
...
class PluginUpdate {
    ...
    private function __construct($initiator) {
        ...
        $this->view = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\view\PluginUpdateView::instance(
            $this
        );
        $this->announcementPool = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\announcement\AnnouncementPool::instance(
            $this
        );
        $this->pluginUpdateChecker = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\PluginUpdateChecker::instance(
            $this
        );
        $this->constructPluginUpdateLicensePool();
    }
    /**
     * Initialize PRO updates for this plugin.
     */
    public function initialize() {
        $licensed = $this->isLicensed();
        $partialLicensed = $this->isPartialLicensed();
        $unlicensed = !$licensed || $partialLicensed;
        ...
        add_action('plugins_loaded', [$this->getPluginUpdateChecker(), 'probablyEnableExternalUpdates']);
        ...
        if ($initiator->isExternalUpdateEnabled() && $unlicensed) {
        ...
        // At least one license is active, enable announcements
        if (!$unlicensed) {
    ...
    public function auto_update_plugin($update, $item) {
        ...
        $initiator = $this->getInitiator();
        if ($item->slug === $initiator->getPluginSlug()) {
     ...
     * @return int[]
     */
    protected function getPotentialBlogIds() {
        // Only needs a license for the main site
        if (
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\Utils::isMu() &&
            !$this->getInitiator()->isMultisiteLicensed()
        ) {
    ...
    public function getFirstInitializationTimestamp() {
        return get_option(
            self::OPTION_NAME_FIRST_INITIALIZATION_PREFIX . $this->getInitiator()->getPluginSlug(),
            \time()
        );
    }
    /**
    ...
    public static function instance($initiator) {
        return new \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\PluginUpdate($initiator);
    }
}
