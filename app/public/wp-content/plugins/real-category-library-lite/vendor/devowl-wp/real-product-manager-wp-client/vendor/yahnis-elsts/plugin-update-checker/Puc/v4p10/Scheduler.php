...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p10_Scheduler', \false)) {
    /**
    ...
    class Puc_v4p10_Scheduler {
         ...
         */
        public function __construct($updateChecker, $checkPeriod, $hourlyHooks = ['load-plugins.php']) {
            ...
            if ($this->checkPeriod > 0) {
                ...
                $defaultSchedules = [1 => 'hourly', 12 => 'twicedaily', 24 => 'daily'];
                ...
                if (!\wp_next_scheduled($this->cronHook) && !\defined('WP_INSTALLING')) {
                    ...
                    $firstCheckTime = \time() - \rand(0, \max($this->checkPeriod * 3600 - 15 * 60, 1));
        ...
        public function upgraderProcessComplete(
            ...
            if (
                !\is_array($upgradeInfo) ||
                !isset($upgradeInfo['type'], $upgradeInfo['action']) ||
                'update' !== $upgradeInfo['action'] ||
                !\in_array($upgradeInfo['type'], ['plugin', 'theme'])
            ) {
                return;
            }
            //Filter out notifications of upgrades that should have no bearing upon whether or not our
            //current info is up-to-date.
            if (\is_a($this->updateChecker, 'DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p10_Theme_UpdateChecker')) {
            ...
            if (\is_a($this->updateChecker, 'DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p10_Plugin_UpdateChecker')) {
