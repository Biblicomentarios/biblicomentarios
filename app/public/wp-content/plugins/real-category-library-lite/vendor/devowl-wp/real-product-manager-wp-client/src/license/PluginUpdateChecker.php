...
use DevOwl\RealCategoryLibrary\Vendor\Puc_v4p4_Plugin_UpdateChecker;
use DevOwl\RealCategoryLibrary\Vendor\Puc_v4_Factory;
use WP_Error;
...
class PluginUpdateChecker {
    ...
    public function probablyEnableExternalUpdates() {
        ...
        require_once \realpath(path_join(__DIR__, '../../vendor/yahnis-elsts/plugin-update-checker/load-v4p10.php'));
        $puc = \DevOwl\RealCategoryLibrary\Vendor\Puc_v4_Factory::buildUpdateChecker(
            $initiator->getHost() . self::ENDPOINT,
            $initiator->getPluginFile(),
    ...
    public function validateResult($response) {
        ...
        if ($license !== \false) {
            $parsedError = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\client\ClientUtils::parseResponse(
                $response
            );
        ...
        return $this->isEnabled()
            ? add_query_arg(
                [
                    ...
                    'puc_slug' => $this->getPluginUpdate()
                        ->getInitiator()
                        ->getPluginSlug(),
                    '_wpnonce' => wp_create_nonce('puc_check_for_updates')
                ],
    ...
    public static function instance($pluginUpdate) {
        return new \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\license\PluginUpdateChecker(
            $pluginUpdate
        );
    }
}
