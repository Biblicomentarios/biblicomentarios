...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\PucReadmeParser', \false)) {
    /**
    ...
    class PucReadmeParser {
        ...
        function parse_readme_contents($file_contents) {
            ...
            $file_contents = \trim($file_contents);
            ...
            foreach (
                ['tags', 'contributors', 'requires_at_least', 'tested_up_to', 'stable_tag', 'donate_link']
                as $chop
            ) {
                if (${$chop}) {
                    $_chop = '_' . $chop;
            ...
            if (!\preg_match('/(^(.*?))^[\\s]*=+?[\\s]*.+?[\\s]*=+?/ms', $file_contents, $_short_description)) {
                $_short_description = [1 => &$file_contents, 2 => &$file_contents];
            }
            ...
            for ($i = 0; $i < \count($_sections); $i += 2) {
                ...
                if (isset($_sections[$i + 1])) {
                    ...
                    $content = $this->filter_text($content, \true);
                } else {
                    $content = '';
                }
                $sections[\str_replace(' ', '_', \strtolower($title))] = ['title' => $title, 'content' => $content];
            }
            // Special sections
            ...
            if (isset($final_sections['upgrade_notice'])) {
                ...
                if (\count($split) >= 2) {
                    for ($i = 0; $i < \count($split); $i += 2) {
                        $upgrade_notice[$this->sanitize_text($split[$i])] = \substr(
                            $this->sanitize_text($split[$i + 1]),
                            0,
                            300
                        );
                    }
            ...
            if (!isset($final_sections['description'])) {
                $final_sections = \array_merge(
                    ['description' => $this->filter_text($_short_description[2], \true)],
                    $final_sections
                );
                $excerpt = \true;
            }
            ...
            $remaining_content = \trim($remaining_content);
            // All done!
        ...
        function user_sanitize($text, $strict = \false) {
            // whitelisted chars
            if (\function_exists('DevOwl\\RealCategoryLibrary\\Vendor\\user_sanitize')) {
                // bbPress native
                return \DevOwl\RealCategoryLibrary\Vendor\user_sanitize($text, $strict);
            }
            if ($strict) {
        ...
        function filter_text($text, $markdown = \false) {
            // fancy, Markdown
            $text = \trim($text);
            $text = \call_user_func([__CLASS__, 'code_trick'], $text, $markdown);
            ...
            if ($markdown) {
                ...
                if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Parsedown', \false)) {
                    ...
                    require_once \dirname(__FILE__) .
                        '/Parsedown' .
                        (\version_compare(\PHP_VERSION, '5.3.0', '>=') ? '' : 'Legacy') .
                        '.php';
                }
                $instance = \DevOwl\RealCategoryLibrary\Vendor\Parsedown::instance();
        ...
        function code_trick($text, $markdown) {
            ...
            if (!$markdown) {
                ...
                $text = \preg_replace_callback('|(`)(.*?)`|', [__CLASS__, 'encodeit'], $text);
                // This gets the "block level" code blocks and converts them to PRE CODE
                $text = \preg_replace_callback("!(^|\n)`(.*?)`!s", [__CLASS__, 'encodeit'], $text);
            } else {
                // Markdown can do inline code, we convert bbPress style block level code to Markdown style
                $text = \preg_replace_callback("!(^|\n)([ \t]*?)`(.*?)`!s", [__CLASS__, 'indent'], $text);
            }
            return $text;
        }
        ...
        function encodeit($matches) {
            if (\function_exists('DevOwl\\RealCategoryLibrary\\Vendor\\encodeit')) {
                // bbPress native
                return \DevOwl\RealCategoryLibrary\Vendor\encodeit($matches);
            }
            $text = \trim($matches[2]);
        ...
        function decodeit($matches) {
            if (\function_exists('DevOwl\\RealCategoryLibrary\\Vendor\\decodeit')) {
                // bbPress native
                return \DevOwl\RealCategoryLibrary\Vendor\decodeit($matches);
            }
            $text = $matches[2];
