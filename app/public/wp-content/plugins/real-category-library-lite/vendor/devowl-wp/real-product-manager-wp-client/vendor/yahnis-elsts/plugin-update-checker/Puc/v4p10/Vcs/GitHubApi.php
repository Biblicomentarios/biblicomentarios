...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p10_Vcs_GitHubApi', \false)) {
    class Puc_v4p10_Vcs_GitHubApi extends \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p10_Vcs_Api {
        /**
        ...
        public function getLatestRelease() {
            ...
            $reference = new \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p10_Vcs_Reference([
            ...
            if ($this->releaseAssetsEnabled && isset($release->assets, $release->assets[0])) {
                ...
                $matchingAssets = \array_filter($release->assets, [$this, 'matchesAssetFilter']);
            ...
            if (!empty($release->body)) {
                ...
                $reference->changelog = \DevOwl\RealCategoryLibrary\Vendor\Parsedown::instance()->text($release->body);
            }
            return $reference;
        }
        /**
        ...
        public function getLatestTag() {
            ...
            $tag = $versionTags[0];
            return new \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p10_Vcs_Reference([
                'name' => $tag->name,
        ...
        public function getLatestCommit($filename, $ref = 'master') {
            $commits = $this->api('/repos/:user/:repo/commits', ['path' => $filename, 'sha' => $ref]);
        ...
        public function getLatestCommitTime($ref) {
            $commits = $this->api('/repos/:user/:repo/commits', ['sha' => $ref]);
            if (!\is_wp_error($commits) && isset($commits[0])) {
         ...
         */
        protected function api($url, $queryParams = []) {
            $baseUrl = $url;
        ...
        protected function buildApiUrl($url, $queryParams) {
            $variables = ['user' => $this->userName, 'repo' => $this->repositoryName];
        ...
        public function getRemoteFile($path, $ref = 'master') {
            ...
            $response = $this->api($apiUrl, ['ref' => $ref]);
        ...
        public function removeAuthHeaderFromRedirects(&$location, &$headers) {
            ...
            if (\strpos($location, $repoApiBaseUrl) === 0) {
                return;
                //This request is going to GitHub, so it's fine.
            }
            //Remove the header.
