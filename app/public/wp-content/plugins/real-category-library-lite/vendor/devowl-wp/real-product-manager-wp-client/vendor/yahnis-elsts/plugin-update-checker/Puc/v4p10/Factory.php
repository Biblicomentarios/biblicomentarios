...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p10_Factory', \false)) {
    /**
    ...
    class Puc_v4p10_Factory {
         ...
         */
        public static function buildFromHeader($fullPath, $args = []) {
        ...
        protected static function isPluginFile($absolutePath) {
            ...
            if (\function_exists('get_file_data')) {
                $headers = \get_file_data($absolutePath, ['Name' => 'Plugin Name'], 'plugin');
        ...
        private static function getServiceURI($fullPath) {
            ...
            if (\is_readable($fullPath)) {
                $seek = ['github' => 'GitHub URI', 'gitlab' => 'GitLab URI', 'bucket' => 'BitBucket URI'];
            ...
            throw new \RuntimeException(\sprintf('Unable to locate URI in header of "%s"', \htmlentities($fullPath)));
        }
        /**
        ...
        private static function getVcsService($metadataUrl) {
            ...
            if (\preg_match($repoRegex, $path)) {
                $knownServices = ['github.com' => 'GitHub', 'bitbucket.org' => 'BitBucket', 'gitlab.com' => 'GitLab'];
