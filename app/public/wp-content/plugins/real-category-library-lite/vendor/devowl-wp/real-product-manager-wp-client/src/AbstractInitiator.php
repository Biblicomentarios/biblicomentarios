...
use DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\Assets;
...
abstract class AbstractInitiator {
     ...
     * @return int[]
     */
    abstract public function getProductAndVariant();
    /**
     ...
     * @return string
     */
    abstract public function getPrivacyPolicy();
    /**
     * Return `true` if you want to enable external updates instead of wordpress.org.
     *
     * @return boolean
     */
    abstract public function isExternalUpdateEnabled();
    /**
    ...
    public function getPluginUpdater() {
        if ($this->pluginUpdate === null) {
            $this->pluginUpdate = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\PluginUpdate::instance(
                $this
            );
        }
    ...
    public function getHost() {
        return \defined('DEVOWL_WP_DEV') && \constant('DEVOWL_WP_DEV')
            ? 'http://real_product_manager_backend:8000/'
            : 'https://license.devowl.io/';
    }
    // Self-explaining
    ...
    public function start() {
        \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\Core::getInstance()->addInitiator($this);
    }
    // Self-explaining
