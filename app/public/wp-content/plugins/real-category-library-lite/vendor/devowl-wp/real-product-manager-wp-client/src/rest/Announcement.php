...
class Announcement {
    ...
    public function rest_api_init() {
        ...
        register_rest_route($namespace, '/announcement/(?P<slug>[a-zA-Z0-9_-]+)/active', [
            'methods' => 'POST',
            'callback' => [$this, 'routeActive'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => ['state' => ['type' => 'boolean', 'required' => \true]]
        ]);
        register_rest_route($namespace, '/announcement/(?P<slug>[a-zA-Z0-9_-]+)/(?P<id>[0-9_-]+)/view', [
            'methods' => 'DELETE',
            'callback' => [$this, 'routeDeleteView'],
            'permission_callback' => [$this, 'permission_callback']
        ]);
    }
    /**
        ...
        if ($initiator === null) {
            ...
            return new \WP_REST_Response([
                'success' => $initiator
                    ->getPluginUpdater()
                    ->getAnnouncementPool()
                    ->isActive($request->get_param('state'))
            ]);
        }
    }
    /**
    ...
    public static function instance() {
        return new \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\rest\Announcement();
    }
}
