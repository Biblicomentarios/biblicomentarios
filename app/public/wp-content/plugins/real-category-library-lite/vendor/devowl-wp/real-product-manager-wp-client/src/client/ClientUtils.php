...
class ClientUtils {
    ...
    public static function parseResponse($response) {
        ...
        if (!empty($body)) {
            ...
            return new \WP_Error(
                self::ERROR_CODE_CLIENT_PARSE,
                __('Something went wrong while parsing an error message from the remote server.'),
                ['response' => $response]
            );
        }
    }
    ...
    public static function request($initiator, $endpoint, $body = null, $method = 'GET') {
        ...
        $response = wp_remote_post($isGet ? add_query_arg($body, $url) : $url, [
            'headers' => [
                'Content-Type' => $isGet ? null : 'application/json; charset=utf-8',
                'Accept-Language' => get_locale()
            ],
            'body' => $isGet ? null : \json_encode($body),
            'method' => $method,
            'data_format' => $isGet ? null : 'body'
        ]);
