...
class Announcement {
    ...
    private function __construct(
        ...
        $id,
        $graphicUrl = null,
        $graphicUrlRetina = null,
        $graphicFileLink = null,
        $title = null,
        $text = null,
        $dismissible = \true,
        $data = [],
    ...
    public function isVisible() {
        return $this->getVisibility() ===
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\announcement\Announcement::VISIBILITY_LOCAL
            ? $this->getPool()
                ->getPluginUpdate()
                ->getInitiator()
    ...
    public static function fromResponse($pool, $response) {
        return new \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\announcement\Announcement(
            $pool,
            $response['id'],
            isset($response['graphicFile']) ? $response['graphicFile']['downloadUrl'] : null,
            isset($response['graphicFile2x']) ? $response['graphicFile2x']['downloadUrl'] : null,
            isset($response['graphicFileLink']) ? $response['graphicFileLink'] : null,
            isset($response['title']) ? $response['title'] : null,
            isset($response['text']) ? $response['text'] : null,
            isset($response['dismissible']) ? $response['dismissible'] : \true,
            isset($response['data']) ? $response['data'] : [],
            isset($response['severity']) ? $response['severity'] : 'info',
            isset($response['visibility']) ? $response['visibility'] : 'local'
        );
    }
}
