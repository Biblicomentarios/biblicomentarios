...
trait CoreLite {
    ...
    public function plugin_action_links($actions) {
        ...
        $link = add_query_arg(
            ...
            $base->getPluginConstant(
                \DevOwl\RealCategoryLibrary\Vendor\DevOwl\Freemium\FreemiumProvider::$PLUGIN_CONST_PRO_VERSION
            )
        );
        $actions[] =
    ...
    public function isLiteNoticeDismissed($set = null) {
        ...
        $base = $this;
        $value = '1';
        $expireOption = new \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\ExpireOption(
            $base->getPluginConstant('OPT_PREFIX') . '_liteDismissed',
            \false,
            20 * \constant('DAY_IN_SECONDS')
        );
        $expireOption->enableTransientMigration(
            \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\ExpireOption::TRANSIENT_MIGRATION_SITE_WIDE
        );
        if ($set !== null) {
