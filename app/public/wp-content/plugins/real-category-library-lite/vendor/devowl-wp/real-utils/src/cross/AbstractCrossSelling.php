...
use DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler;
...
abstract class AbstractCrossSelling {
    ...
    public function getAssetsUrl($path = '') {
        return \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getBaseAssetsUrl(
    ...
    public function actionCounter($action, $increment = \false) {
        $optionName =
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_CROSS_COUNTER .
            '.' .
            $this->getSlug() .
            '.' .
            $action;
        $cnt = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::get(
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_INITIATOR_CROSS,
            $optionName,
            0
        );
        if ($increment) {
    ...
    public function forceHide($action, $force = \false) {
        $skip = \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::get(
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_INITIATOR_CROSS,
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_CROSS_SKIP,
            []
        );
        if ($force) {
    ...
    public function dismiss($action, $force) {
        ...
        if (
            \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::set(
                \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_INITIATOR_CROSS,
                \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_CROSS_SELLING,
                $ts
            )
        ) {
            return $ts;
