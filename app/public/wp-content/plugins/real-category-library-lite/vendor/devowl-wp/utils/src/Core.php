...
trait Core {
    ...
    protected function construct() {
        ...
        $pluginFile = $this->getPluginConstant(
            \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_FILE
        );
        // Register immediate actions and filters
        $this->activator = $this->getPluginClassInstance(
            \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CLASS_ACTIVATOR
        );
        $this->assets = $this->getPluginClassInstance(
            \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CLASS_ASSETS
        );
        $this->service = \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\Service::instance($this);
        ...
        $this->getPluginClassInstance(
            \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CLASS_LOCALIZATION
        )->hooks();
        \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PackageLocalization::instance(
            $this->getPluginConstant(
                \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_ROOT_SLUG
            ),
            \dirname(__DIR__)
        )->hooks();
    ...
    public function i18n() {
        load_plugin_textdomain(
            $this->getPluginConstant(
                \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_TEXT_DOMAIN
            ),
            \false,
            \dirname(
                plugin_basename(
    ...
    public function updateDbCheck() {
        ...
        if (
            $installed !==
            $this->getPluginConstant(
                \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_VERSION
            )
        ) {
            ...
            do_action(
                'DevOwl/Utils/NewVersionInstallation/' .
                    $this->getPluginConstant(
                        \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_SLUG
                    ),
                $installed
            );
        }
    ...
    public function getInternalPackages() {
        ...
        $globPath = path_join(
            $this->getPluginConstant(
                \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_PATH
            ),
            'vendor/' .
                $this->getPluginConstant(
                    \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\Utils\PluginReceiver::$PLUGIN_CONST_ROOT_SLUG
