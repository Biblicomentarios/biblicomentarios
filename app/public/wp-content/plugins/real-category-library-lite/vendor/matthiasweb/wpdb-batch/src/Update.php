...
namespace DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\WpdbBatch;

/**
 * Batch update implementation.
 */
class Update extends \DevOwl\RealCategoryLibrary\Vendor\MatthiasWeb\WpdbBatch\AbstractBatch {
    /**
            ...
            foreach ($values as $key => $val) {
                ...
                foreach (\array_keys($val) as $field) {
                    if ($field !== $index) {
                        ...
                        $final[$field][] =
                            'WHEN `' . $this->columnEscape($index) . '` = ' . $valIndex . ' THEN ' . $value . ' ';
                    }
                }
            }
