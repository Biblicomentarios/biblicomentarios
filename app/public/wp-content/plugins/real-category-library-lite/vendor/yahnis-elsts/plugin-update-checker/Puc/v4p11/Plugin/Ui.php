...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p11_Plugin_Ui', \false)) {
    /**
    ...
    class Puc_v4p11_Plugin_Ui {
        ...
        public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
            if ($this->isMyPluginFile($pluginFile)) {
                $linkUrl = \wp_nonce_url(
                    \add_query_arg(
                        ['puc_check_for_updates' => 1, 'puc_slug' => $this->updateChecker->slug],
        ...
        public function handleManualCheck() {
            ...
            if ($shouldCheck) {
                ...
                \wp_redirect(
                    \add_query_arg(
                        ['puc_update_check_result' => $status, 'puc_slug' => $this->updateChecker->slug],
                        \self_admin_url('plugins.php')
                    )
                );
                exit();
        ...
        public function displayManualCheckResult() {
            if (
                isset($_GET['puc_update_check_result'], $_GET['puc_slug']) &&
                $_GET['puc_slug'] == $this->updateChecker->slug
            ) {
                ...
                if ($status == 'no_update') {
                    $message = \sprintf(
                        \_x('The %s plugin is up to date.', 'the plugin title', 'plugin-update-checker'),
                        $title
                    );
                } else {
                    if ($status == 'update_available') {
                        $message = \sprintf(
                            \_x(
                                'A new version of the %s plugin is available.',
                                'the plugin title',
        ...
        protected function formatManualCheckErrors($errors) {
            ...
            foreach ($errors as $item) {
                $wpError = $item['error'];
                /** @var WP_Error $wpError */
                $output .= \sprintf($formatString, $wpError->get_error_message(), $wpError->get_error_code());
            }
            if ($showAsList) {
                $output .= '</ol>';
            }
