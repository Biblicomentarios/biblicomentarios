...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p11_DebugBar_Extension', \false)) {
    class Puc_v4p11_DebugBar_Extension {
        ...
        public function __construct($updateChecker, $panelClass = null) {
            ...
            \add_filter('debug_bar_panels', [$this, 'addDebugBarPanel']);
            \add_action('debug_bar_enqueue_scripts', [$this, 'enqueuePanelDependencies']);
            \add_action('wp_ajax_puc_v4_debug_check_now', [$this, 'ajaxCheckNow']);
        }
        /**
        ...
        public function enqueuePanelDependencies() {
            ...
            \wp_enqueue_script('puc-debug-bar-js-v4', $this->getLibraryUrl('/js/debug-bar.js'), ['jquery'], '20201209');
        }
        /**
        ...
        public function ajaxCheckNow() {
            ...
            if ($update !== null) {
                ...
                echo '<pre>', \htmlentities(\print_r($update, \true)), '</pre>';
            } else {
            ...
            if (!empty($errors)) {
                \printf(
                    '<p>The update checker encountered %d API error%s.</p>',
                    \count($errors),
                    \count($errors) > 1 ? 's' : ''
                );
                    ...
                    if (isset($item['httpResponse'])) {
                        if (\is_wp_error($item['httpResponse'])) {
                            ...
                            if ($body === '') {
                                ...
                                if (\strlen($body) > self::RESPONSE_BODY_LENGTH_LIMIT) {
                                    $length = \strlen($body);
                                    $body =
                                        \substr($body, 0, self::RESPONSE_BODY_LENGTH_LIMIT) .
        ...
        public function removeHooks() {
            \remove_filter('debug_bar_panels', [$this, 'addDebugBarPanel']);
            \remove_action('debug_bar_enqueue_scripts', [$this, 'enqueuePanelDependencies']);
            \remove_action('wp_ajax_puc_v4_debug_check_now', [$this, 'ajaxCheckNow']);
        }
        /**
        ...
        private function getLibraryUrl($filePath) {
            ...
            $absolutePath = \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p11_Factory::normalizePath($absolutePath);
            $pluginDir = \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p11_Factory::normalizePath(\WP_PLUGIN_DIR);
            $muPluginDir = \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p11_Factory::normalizePath(\WPMU_PLUGIN_DIR);
            $themeDir = \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p11_Factory::normalizePath(\get_theme_root());
            if (\strpos($absolutePath, $pluginDir) === 0 || \strpos($absolutePath, $muPluginDir) === 0) {
                //It's part of a plugin.
                return \plugins_url(\basename($absolutePath), $absolutePath);
            } else {
                if (\strpos($absolutePath, $themeDir) === 0) {
