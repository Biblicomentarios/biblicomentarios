...
if (
    !\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p11_DebugBar_Panel', \false) &&
    \class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Debug_Bar_Panel', \false)
) {
    class Puc_v4p11_DebugBar_Panel extends \DevOwl\RealCategoryLibrary\Vendor\Debug_Bar_Panel {
        ...
        private function displayConfiguration() {
            ...
            $this->row(
                'Metadata URL',
                \htmlentities($this->updateChecker->metadataUrl) . ' ' . $requestInfoButton . $this->responseBox
            );
        ...
        private function displayStatus() {
            ...
            if (\function_exists('get_submit_button')) {
                $checkNowButton = \get_submit_button('Check Now', 'secondary', 'puc-check-now-button', \false, [
                    'id' => $this->updateChecker->getUniqueName('check-now-button')
                ]);
            }
            if ($state->getLastCheck() > 0) {
                $this->row(
                    'Last check',
                    $this->formatTimeWithDelta($state->getLastCheck()) . ' ' . $checkNowButton . $this->responseBox
                );
            } else {
        ...
        protected function getUpdateFields() {
            return ['version', 'download_url', 'slug'];
        }
        private function formatTimeWithDelta($unixTime) {
            ...
            $delta = \time() - $unixTime;
            $result = \human_time_diff(\time(), $unixTime);
            if ($delta < 0) {
                $result = 'after ' . $result;
            } else {
                $result = $result . ' ago';
        ...
        public function row($name, $value) {
            if (\is_object($value) || \is_array($value)) {
                $value = '<pre>' . \htmlentities(\print_r($value, \true)) . '</pre>';
            } else {
                if ($value === null) {
