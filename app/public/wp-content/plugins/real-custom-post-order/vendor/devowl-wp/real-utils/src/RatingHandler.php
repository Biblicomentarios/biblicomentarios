...
class RatingHandler {
    ...
    public function dismiss($slug, $force) {
        ...
        $initiator = \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiator($slug);
        if ($initiator === null) {
            return \false;
        }
        ...
        if (
            \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\TransientHandler::set(
                $initiator,
                \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING,
                $ts
            )
        ) {
            return $ts;
    ...
    public function getCanBeRated() {
        ...
        $cache = [];
        foreach (
            \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiators()
            as $initiator
        ) {
            $ts = \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\TransientHandler::get(
                $initiator,
                \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING
            );
            // Initialize default
    ...
    public static function instance() {
        return new \DevOwl\RealCustomPostOrder\Vendor\DevOwl\RealUtils\RatingHandler();
    }
}
