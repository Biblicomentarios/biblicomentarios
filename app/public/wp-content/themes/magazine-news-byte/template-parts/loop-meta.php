		...
		<div <?php hoot_attr('loop-meta-wrap', 'archive', $wrap_attr); ?>>
			...
			<div class="hgrid">
				...
				<div <?php hoot_attr('loop-meta', 'archive', 'hgrid-span-12'); ?>>
					...
					<h1 <?php hoot_attr('loop-title', 'archive'); ?>><?php echo wp_kses_post(get_the_archive_title()); // Displays title for archive type (multi post) pages. 
																		?></h1>
				...
				<div class="bg-dark text-white text-center">
					...
					if (get_field('urlanterior', $term)) {
					?>
						<a class="text-white" href="<?= get_field('urlanterior', $term) ?>">Anterior</a>
					<?php
					}

					if (get_field('urlsiguiente', $term)) {
					?>
						|
						<a class="text-white" href="<?= get_field('urlsiguiente', $term) ?>">Siguiente</a>
					<?php
					}
					?>
				</div>
						...
						<div <?php hoot_attr('loop-meta', '', 'hgrid-span-12'); ?>>
							<div class="entry-header">
								...
								?><div <?php hoot_attr('loop-description'); ?>><?php
																	...
																	elseif (!$hide_meta_info && 'top' == hoot_get_mod('post_meta_location') && !is_attachment() && !(function_exists('is_buddypress') && is_buddypress())) :
