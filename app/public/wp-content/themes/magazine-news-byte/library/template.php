...
function hoot_display_meta_info( $args = '', $context = '', $editlink = true ) {
	...
	if ( !empty( $display['author'] ) ) :
		$blocks['author']['label'] = __( 'Por:', 'magazine-news-byte' );
		ob_start();
	...
	if ( !empty( $display['date'] ) ) :
		$blocks['date']['label'] = __( 'Fecha:', 'magazine-news-byte' );
	...
	if ( !empty( $display['cats'] ) ) :
		...
		if ( !empty( $category_list ) ) :
			$blocks['cats']['label'] = __( 'En:', 'magazine-news-byte' );
			$blocks['cats']['content'] = $category_list;
		endif;
	endif;

	if ( !empty( $display['tags'] ) && get_the_tags() ) :
		$blocks['tags']['label'] = __( 'Etiquetas:', 'magazine-news-byte' );
		$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'magazine-news-byte' ) : get_the_tag_list( '', ', ', '' );
	endif;

	if ( !empty( $display['comments'] ) && comments_open() ) :
		$blocks['comments']['label'] = __( 'Con:', 'magazine-news-byte' );
		ob_start();
		comments_popup_link(__( '0 Comentarios', 'magazine-news-byte' ),
							__( '1 Comentario', 'magazine-news-byte' ),
							__( '% Comentarios', 'magazine-news-byte' ), 'comments-link', '' );
...
function hoot_developer_data( $commented = true ) {
	...
	timer_stop(1);
	echo esc_html( __( ' segundos. ', 'magazine-news-byte' ) . ( memory_get_peak_usage(1) / 1024 ) / 1024 . 'MB' );
	$out = ob_get_contents();
	ob_end_clean();
	return $out;
}
